...
my $READS="$ASSEMBLY/reads.txt-rev3";
...
# Process read counts
my %reads;
open(IN,$READS) || die $READS;
while(<IN>) {
  chomp; my @fields=split; next unless @fields>=4;
  my ($indiv,$hap,$altTrans,$reads)=@fields;
  if($altTrans=~/(\S+)_(\S+)_(\d+)/) { $altTrans="$1\_$2" }
  $reads{"$indiv $hap $altTrans"}=$reads
}
...
while(<IN>) {
  ...
  if($transcript=~/^ALT\d+_(\S+)/) {
    ...
    my $reads=$reads{"$indiv $allele $transcript"};
    if($reads>=$MIN_READS) {
      $expressedALT{$id}=1;
    }
