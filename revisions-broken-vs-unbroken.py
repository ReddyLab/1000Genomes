...
def load(filename):
    ...
    with open(filename,"rt") as fh:
        while(True):
            ...
            if(int(geneReads)==0 or int(reads)==0): continue
            if(sum.get(gene,None)==None):
                sum[gene]=0.0
                sampleSizes[gene]=0
            sum[gene]+=float(reads)/float(geneReads)
            sampleSizes[gene]+=1
    hash={}
    keys=sum.keys()
    for gene in keys:
        hash[gene]=sum[gene]/float(sampleSizes[gene])
    return hash
...
for gene in genes:
    ...
    unbrokenValue=unbroken.get(gene,0.0)
    if(brokenValue==0.0 and unbrokenValue==0.0): continue
