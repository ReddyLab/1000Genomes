...
ID_LIST=RNA+"/IDs.txt"
SRC=THOUSAND+"/src"
PROGRAM=SRC+"/find-two-broken.py"
SLURM_DIR=THOUSAND+"/two-broken-slurms"
JOB_NAME="BROKEN"
MAX_PARALLEL=10000
NICE=500
MEMORY=5000
...
writer=SlurmWriter()
indivs=os.listdir(THOUSAND+"/combined")
for indiv in indivs:
    ID=indiv.rstrip()
    if(not os.path.exists(THOUSAND+"/combined/"+ID+"/1-filtered-fixed.essex")): continue
    for hap in (1,2):
        outfile=THOUSAND+"/two-broken/"+ID+"."+str(hap)+".txt"
